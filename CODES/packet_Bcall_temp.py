	...
	def move_robot(self, linear_velocity, angular_velocity):
        ...
        if angular_velocity != 0:
            if angular_velocity > 0:  # Turn right
                l_pwm = int(angular_velocity + 22)
                r_pwm = int(-(angular_velocity + 22))
            else:  # Turn left
                l_pwm = int(-(angular_velocity + 22))
                r_pwm = int(angular_velocity + 22)
...
def switch_check():
    switch_state = GPIO.input(BUTTON_PIN)
     print("Initial Switch state:", "OFF" if switch_state == GPIO.LOW else "ON")
    while True:
        ...
        if ser in rlist:
            ...
            if b"Percentage" in res:
                try:
                    ...
                    if len(split_res) >= 2:
                        ...
                        #print(f"Battery Percentage: {percentage_int}%")
                    else:
            ...
            if b"Temperature" in res:
                try:
                    ...
                    split_res = decoded_res.split(":")
                    if len(split_res) == 2:
                        Temp = float(split_res[1].split("C")[0].strip())
                    ...
                    if ticks[0] > 0:
                        ...
                        if time_interval > 0:
                            ...
                            if 0 <= speed <= 100:  # Adjust the threshold as needed
                                ...
                                robot_status = 1
                                print(f"Total distance traveled: {forward_distance:.2f} cm, Current Speed is: {speed:.2f} m/sec")
            ...
            if current_time - last_packet_time >= 5:  # Send packet for every 5 seconds
                ...
                packet = {
                            ...
                            "battery_status": f"{percentage_int}%",
                            "robot_status": robot_status,
                            "Bcall": immobilizer.Bcall,
