...
class Immobilizer:
    ...
    def message_received_handler(self, message):
        data = message.data.decode('utf-8')  #Decoding bytes to string
...
def read_serial(ser):
    ...
    total_distance_reverse = 0
    WHEEL_CIRCUMFERENCE_CM = 26.3  # One turn
    prev_ticks = [0, 0, 0, 0]
    counter = 0
    forward_distance = 0
    reverse_distance = 0
    ...
    while True:
        ...
        if len(split_data) >= 4:
            ...
            # Construct the message body to include only total forward and reverse distance
            message_body = f"Total forward distance: {forward_distance} cm"

            # Sending the message to Azure IoT Hub
